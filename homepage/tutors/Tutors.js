...
/* =====================================================
   ...
   Handles: tutor API (Tutor Registration sheet data),
   card creation, responsive card count, seamless carousel,
   prev/next, auto rotation,
...
(function () {
  ...
  async function loadTutors() {
    ...
    /*
     * FIX: guarantee the spinner can never spin forever.
     *
     * Previously a hung network request (slow connection, a
     * stale/incorrect deployment URL, etc.) meant the fetch()
     * promise never resolved or rejected in a timely way, so
     * "Loading tutors..." stayed on screen indefinitely. An
     * AbortController now forces the request to fail after
     * 15 seconds, which the existing catch block below turns
     * into the normal error state.
     */
  ...
  function getTutorName(tutor) {
    ...
    return cleanValue(tutor.fullName) || "Professional Tutor";
  }
